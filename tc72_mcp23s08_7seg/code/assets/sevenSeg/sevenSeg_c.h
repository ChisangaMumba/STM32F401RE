...
{
	mcp23s08_init(DIGIT_ADDR, 0x00);
	mcp23s08_init(COLUMN_ADDR, 0x00);
...
void writeChar(char chr, uint8_t chrPos, int8_t dpPos)
{
	...
	serial_digitSend(~digit, chrPos);  // charset is for common cathode, thus inversion of byte to conform to common anode
}
//------------------------------------------------------------------------------
//													ascii to 7 segment
//------------------------------------------------------------------------------
...
{ 
	mcp23s08_write(DIGIT_ADDR, OLAT, digit);  
	delay_ms(2);
	mcp23s08_write(COLUMN_ADDR, OLAT, 0x01 << chrPos);
}
//------------------------------------------------------------------------------
#endif
