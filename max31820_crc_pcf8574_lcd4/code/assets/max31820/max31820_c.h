...
#define	MAX31820_C
//------------------------------------------------------------------------------
#include "max31820_h.h"
//------------------------------------------------------------------------------
///*
void max31820_init(int8_t highTemp, int8_t lowTemp, uint8_t res, max31820_t *sensor)
{
	pinMode(DQ, OUTPUT, OPEN_DRAIN, NO_PU_PD);
	
	max31820_command(WRITE_SCRATCHPAD);

  max31820_writeByte(highTemp);	// high temperature threshold for thermostat
  max31820_writeByte(lowTemp);		// low temperature threshold for thermostat
  max31820_writeByte(res);     	// ADC resolution setting
	
	max31820_getSensorData(sensor);
}
//------------------------------------------------------------------------------
uint8_t max31820_reset(void) 
{
...
//------------------------------------------------------------------------------
void max31820_writeBit(uint8_t bit) 
{
	digitalWrite(DQ, LOW);					// DQ = 0; // Pull low
...
//------------------------------------------------------------------------------
void max31820_writeByte(uint8_t byte) 
{
	for(uint8_t i = 0; i < 8; i++) 
	{
		max31820_writeBit((byte >> i) & 0x01);
	}
}
//------------------------------------------------------------------------------
uint8_t max31820_readByte(void) 
{
	uint8_t byte = 0;
...
//------------------------------------------------------------------------------
float max31820_readTemp(void) 
{
	uint8_t byte[2];
	
	max31820_command(CONVERT_TEMP);
	max31820_command(READ_SCRATCHPAD);

	byte[0] = max31820_readByte();  	// LSB
	byte[1] = max31820_readByte();		// MSB
...
//------------------------------------------------------------------------------
uint8_t max31820_getSensorData(max31820_t *sensor)
{
	uint8_t data[9];
	
  if(!max31820_command(CONVERT_TEMP))
  {
    return 0;                      // return 0 if error
  }
  ...
  for(uint8_t i = 0; i < 9; i++)
  {
	...
	if(!max31820_isDataValid(data, 8))
		return 0;
	
	max31820_setFields(sensor, data);
...
//------------------------------------------------------------------------------
uint8_t max31820_isDataValid(uint8_t data[], uint8_t size)
{
	uint8_t isValid = 0;
	
	if(max31820_calcCRC(data, size) == data[8])	// data[8] == CRC value
	{
		isValid = 1;
	}
...
//------------------------------------------------------------------------------
void max31820_setFields(max31820_t *sensor, uint8_t data[])
{
...
//------------------------------------------------------------------------------
void max31820_convDelay(uint8_t resInd)
{
	switch(resInd) 
	{
