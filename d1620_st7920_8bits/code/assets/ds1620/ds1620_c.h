...
{
	uint8_t rawConfig = ds1620_read(READ_CONFIG);

	if(sensor->isSet == 0)
	{
		sensor->tempHigh = ds1620_getTemp(READ_TH);
		sensor->tempLow = ds1620_getTemp(READ_TL);
		sensor->tempHFlag = (rawConfig & 0x40) >> 6;
		sensor->tempLFlag = (rawConfig & 0x20) >> 5;
		
		sensor->isSet = 1;
	}
	
	sensor->temp = ds1620_getTemp(READ_TEMP);
}
//------------------------------------------------------------------------------
