...
void internalTempSensor_init(void)
{
	...
	// Select the ADC channel 16 or 18(internal temperature sensor)
	ADC->CCR |= ADC_CCR_TSVREFE;          // Enable temperature sensor (Temperature Sensor and VREFINT Enable)
	ADC1->SQR3 = 16;                      // Channel 16 or 18(internal temperature sensor)
	ADC1->SMPR1 |= 0;  										// Sampling time of 3 cycles

	// Enable ADC1
	ADC1->CR2 |= ADC_CR2_ADON;
}
//------------------------------------------------------------------------------
uint32_t internalTempSensor_rawValue(void)
{
	uint32_t result = 0;
	
	ADC1->CR2 |= ADC_CR2_SWSTART; // Start conversion
	while(!(ADC1->SR & 2));  			// Wait for conversion to complete
	result = ADC1->DR;       			// Read conversion

	return result;
}
//------------------------------------------------------------------------------
float internalTempSensor_calcTemp(void)
{
	float v_sense, tempr;
	
	uint32_t adc_value = internalTempSensor_rawValue();
	// Formula from STM32F401RE reference manual
	v_sense = (adc_value * 3.3f) / 4095;        // Convert ADC value to voltage                
	tempr = ((v_sense - 0.76f) / 0.0025f) + 25;  // Convert to temperature (Celsius)
