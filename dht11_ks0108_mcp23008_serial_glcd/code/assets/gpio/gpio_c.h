...
#include "gpio_h.h"
//------------------------------------------------------------------------------
void clock_init(void)  //Set Clock to 16Mhz
{
...
void pinMode(uint16_t portPin, uint8_t mode, uint8_t oType, uint8_t pupd)
{
	...
	// Pull-Up Pull-Down Register (PUPDR):  No pull-up (reset state), pull-down;  Pull-up;  Pull-down
...
//------------------------------------------------------------------------------
void pinAltFunc(uint16_t portPin, uint8_t oType, uint8_t pupd, uint8_t altFunc)
{
	uint8_t port = portPin / 16;
	uint8_t pin = portPin % 16;
	GPIO_TypeDef * GPIO = getGPIO(port);
	...
	// Mode Register (MODER): input, output, Alt function, analogue
	GPIO->MODER &= ~(0x03UL << (pin*2));	// Resetting the value of the bits
	GPIO->MODER |=  (ALTFUN << pin*2); 		// Setting up bit value to the mode
	...
	// Output Speed Register (OSPEEDR): Low, medium, fast, high
	GPIO->OSPEEDR &= ~(0x03UL << (pin*2)); 		// Resetting the value of the bits
	GPIO->OSPEEDR |= (HIGH_SPEED << (pin*2)); // Setting up bit value
	...
	uint8_t afrReg = pin / 8;		// (0 for pins 0–7(AFRL), 1 for pins 8–15(AFRH)) 
	uint8_t pinPos = pin % 8; 	// Pin positions range from 0 - 7 in each Register
	
	GPIO->AFR[afrReg] &= ~(0x0F << (pinPos * 4)); // 4 bits offset within AFRx
	GPIO->AFR[afrReg] |= (altFunc << (pinPos * 4)); 
}
//------------------------------------------------------------------------------
