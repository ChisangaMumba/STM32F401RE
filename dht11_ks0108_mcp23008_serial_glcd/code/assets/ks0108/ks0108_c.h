...
{   
	mcp23008_init(ctrl_addr, 0x00); // Initialize pcf8575 with all pins low (outputs)
	mcp23008_init(data_addr, 0x00); 
...
void ks0108_command(uint8_t command, uint8_t chipNo)
{
	...
	mcp23008_write(ctrl_addr, OLAT, ctrlCommands); 
  mcp23008_write(data_addr, OLAT, command);   
...
void ks0108_data(uint8_t data, uint8_t chipNo)
{
	...
	mcp23008_write(ctrl_addr, OLAT, ctrlCommands); 
  mcp23008_write(data_addr, OLAT, data);
...
{
	mcp23008_write(ctrl_addr, OLAT, ctrlData | EN_HIGH);
	mcp23008_write(ctrl_addr, OLAT, ctrlData & EN_LOW);
}
//------------------------------------------------------------------------------
