...
void writeChar(char chr, uint8_t chrPos, int8_t dpPos)
{
	...
	uint16_t colSelect = ~(0x01 << chrPos);		// Column is the higher byte. Since Common Cathode used here, use zero to activate digit
	
	shiftReg_digitSend((colSelect << 8) | digit); // High byte | Low byte
}
//------------------------------------------------------------------------------
//													ascii to 7 segment
//------------------------------------------------------------------------------
