...
void i2c_init(void)
{
  ...
  pinMode(SDAT, INPUT, OPEN_DRAIN); // SDAT = 1
  digitalWrite(SCLK, HIGH);   			// SCLK = 1
}
//------------------------------------------------------------------------------ 
void i2c_start(void)
{
  pinMode(SDAT, OUTPUT, OPEN_DRAIN);  // SDAT = 0
...
{
  pinMode(SDAT, (ackn == 1) ? INPUT : OUTPUT, OPEN_DRAIN);  // SDAT = ackn; 
  digitalWrite(SCLK, HIGH);                      						// SCLK = 1
  digitalWrite(SCLK, LOW);                       						// SCLK = 0
  pinMode(SDAT, INPUT, OPEN_DRAIN);                         // SDAT = 1
}
...
{	
  ...
  {
    pinMode(SDAT, ((data << tx) & 0x80) ? INPUT : OUTPUT, OPEN_DRAIN) ; //MSB first SDAT 1 or 0
    digitalWrite(SCLK, HIGH);   // SCLK = 1
