...
{     
  max6954_write(DECODE_MODE_REG, DISABLE_DECODE, chip);    
...
{
	uint8_t chip = chrPos / 16; // Determine which chip to write to based on character position. Max6954 is a 16 digit driver in 7 segment mode
  uint8_t chrAddr = chrPos % 16; // valid character positions range from 0-15 for each chip
	uint8_t digit = getDigit(chr, chrPos, dpPos);
	
	max6954_write(DISPLAY_REG0 + chrAddr, digit, chip);	// character address is 0-15 for each chip. i.e 0x20, 0x21, 0x22, ... 0x2F
}
//------------------------------------------------------------------------------
