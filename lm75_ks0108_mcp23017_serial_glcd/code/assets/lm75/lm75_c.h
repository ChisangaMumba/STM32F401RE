...
float toFloat(int16_t rawData)
{
  float tempr = rawData >> 8;      // Get the integer part  
	int16_t fracBits = rawData & FRAC_FLAG; 
	
	if(fracBits)
	{		
		tempr += ((fracBits >> 5) * 0.125); // Get the fractional part. 2^(-3) = 0.125 (result is in multiples of 0.125 ie LSBit for an 11-bit res ADC) 
	}
