...
{   
	mcp23017_init(0x00, 0x00); // Initialize mcp23008 with all pins low (outputs)
...
void ks0108_command(uint8_t command, uint8_t chipNo)
{
  ...
  mcp23017_write(OLAT_A, ctrlCommands);          
	mcp23017_write(OLAT_B, command);
...
{
	mcp23017_write(OLAT_A, ctrlData | EN_HIGH);
	mcp23017_write(OLAT_A, ctrlData & EN_LOW);
}
//------------------------------------------------------------------------------
