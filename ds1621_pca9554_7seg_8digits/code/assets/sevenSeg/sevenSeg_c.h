...
void writeChar(char chr, uint8_t chrPos, int8_t dpPos)
{
	...
	uint8_t colSelect = ~(0x01 << chrPos);		// Since Common Cathode used here, use zero to activate digit
	
	serial_digitSend(digit, colSelect);
}
//------------------------------------------------------------------------------
void serial_digitSend(uint8_t digit, uint8_t colSelect)
{ 
	pca9554_write(DIGIT_ADDR, PORT_OUT_REG, digit);
	delay_ms(3);
