...
{   
	pca95xx_init(0x00, 0x00); // Initialize pcf8575 with all pins low (outputs)
...
void ks0108_command(uint8_t command, uint8_t chipNo)
{
  ...
  pca95xx_write16(PORTA_WR, ctrlCommands, command);          
	ks0108_latch(ctrlCommands, command);
}
//------------------------------------------------------------------------------
void ks0108_data(uint8_t data, uint8_t chipNo)
{
  ...
  pca95xx_write16(PORTA_WR, ctrlCommands, data);          	
	ks0108_latch(ctrlCommands, data);
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void ks0108_string(uint8_t row, uint8_t col, uint8_t fontNo, char str[])
{
...
//------------------------------------------------------------------------------
void ks0108_latch(uint8_t ctrlData, uint8_t data) 
{
	pca95xx_write16(PORTA_WR, ctrlData | EN_HIGH, data);
	pca95xx_write16(PORTA_WR, ctrlData & EN_LOW, data);
}
//------------------------------------------------------------------------------
