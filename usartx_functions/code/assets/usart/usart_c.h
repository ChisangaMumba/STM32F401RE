...
{
	while(!(USARTx->SR & USART_SR_TXE));	// wait until TX buffer empty
	USARTx->DR = chr;
...
int usart_rx(void)
{
	while(!(USARTx->SR & USART_SR_RXNE));	// wait until char arrives
	int chr = USARTx->DR;
...
{
	int c = usart_tx(chr); // Write the character to console
...
int fgetc(FILE *f) 
{
	int chr = usart_rx(); // Read the character from console
