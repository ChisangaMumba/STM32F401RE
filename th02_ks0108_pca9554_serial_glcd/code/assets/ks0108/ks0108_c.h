...
{   
	// Initialize pca9554 with all pins low (outputs)
	pca9554_init(CTRL_ADDR, 0x00);
	pca9554_init(DATA_ADDR, 0x00);
...
void ks0108_command(uint8_t command, uint8_t chipNo)
{
  ...
  pca9554_write(CTRL_ADDR, PORT_OUT_REG, ctrlCommands);          
	pca9554_write(DATA_ADDR, PORT_OUT_REG, command);
...
void ks0108_data(uint8_t data, uint8_t chipNo)
{
  ...
  pca9554_write(CTRL_ADDR, PORT_OUT_REG, ctrlCommands);          
	pca9554_write(DATA_ADDR, PORT_OUT_REG, data);
...
{
	pca9554_write(CTRL_ADDR, PORT_OUT_REG, ctrlData | EN_HIGH);
	pca9554_write(CTRL_ADDR, PORT_OUT_REG, ctrlData & EN_LOW);
}
//------------------------------------------------------------------------------
