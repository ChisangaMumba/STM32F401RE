...
{   
	pcf8574_init(ctrl_addr, 0x00); // Initialize pcf8575 with all pins low (outputs)
	pcf8574_init(data_addr, 0x00); 
...
void ks0108_command(uint8_t command, uint8_t chipNo)
{
	...
	pcf8574_write(ctrl_addr, ctrlCommands); 
  pcf8574_write(data_addr, command);   
...
{
	pcf8574_write(ctrl_addr,  ctrlData | EN_HIGH);
	pcf8574_write(ctrl_addr,  ctrlData & EN_LOW);
}
//------------------------------------------------------------------------------
