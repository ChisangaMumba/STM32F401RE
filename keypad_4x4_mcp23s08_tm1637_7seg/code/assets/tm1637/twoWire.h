...
{	
	pinMode(CLK, OUTPUT, PUSH_PULL, PULL_UP);

  digitalWrite(CLK, LOW);   										// CLK = 0
  pinMode(DIO, OUTPUT, OPEN_DRAIN, PULL_UP);		// DIO = 0
}
//------------------------------------------------------------------------------
void twoWire_start(void)
{	
	digitalWrite(CLK, HIGH);											// CLK = 1;
  pinMode(DIO, INPUT, OPEN_DRAIN, PULL_UP);	  	// DIO = 1;
  delay_us(2);
  pinMode(DIO, OUTPUT, OPEN_DRAIN, PULL_UP);	  // DIO = 0;
}
//------------------------------------------------------------------------------
...
void twoWire_write(uint8_t data)
{
  ...
  {		
	  digitalWrite(CLK, LOW);		// CLK = 0;
	  pinMode(DIO, ((data >> tx) & 0x01) ? INPUT : OUTPUT, OPEN_DRAIN, PULL_UP);   // LSB first (Real 12c sends MSB first)   
