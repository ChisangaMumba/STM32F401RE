...
void lcd_init(void)
{  
  pcf8574_init(PCF8574_ADDR, 0x00); // Initialize PCF8574 with all pins low (outputs)
...
//------------------------------------------------------------------------------
void lcd_string(uint8_t row, uint8_t column, char str[])
{
...
void lcd_sendNibble(uint8_t data)
{
  ...
  pcf8574_write(PCF8574_ADDR, data | EN_high);
  delay_ms(2);
  pcf8574_write(PCF8574_ADDR, data & EN_low);
}
//------------------------------------------------------------------------------
#endif
