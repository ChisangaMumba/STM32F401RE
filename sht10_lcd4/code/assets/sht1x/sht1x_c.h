...
{
	uint16_t raw = sht1x_read(TEMP_CMD);
	
	return (-39.6 + 0.01 * (float)raw);
}
//------------------------------------------------------------------------------
...
{
	float rh_linear;
	uint16_t rawHumd = sht1x_read(HUMD_CMD);
	
	rh_linear = -2.0468 + 0.0367 * rawHumd + (-1.5955e-6) * rawHumd * rawHumd;
	
	return (tempr - 25.0) * (0.01 + 0.00008 * rawHumd) + rh_linear;
}
//------------------------------------------------------------------------------
#endif
