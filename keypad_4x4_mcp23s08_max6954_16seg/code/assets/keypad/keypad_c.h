...
{
	mcp23s08_init(KEYPAD_ADDR, 0xF0);  // 4 inputs (cols), 4 outputs (rows)
}
//------------------------------------------------------------------------------
inline void keypad_rowSelect(uint8_t data)
{ 
	mcp23s08_write(KEYPAD_ADDR, OLAT, data);
}
//------------------------------------------------------------------------------
inline uint8_t keypad_readInputs(void)
{
	return mcp23s08_read(KEYPAD_ADDR);
}
//------------------------------------------------------------------------------
int8_t keypad_scan(void)
{
  int8_t keyIndex = -1;  // invalid array index
	...
	for(uint8_t row = 0; row < 4; row++)
	{
		...
		if(keyPins & COL3)
		{
			keyIndex = indices[row][3];		// ROWn, COL3
			break;
		}
	}
