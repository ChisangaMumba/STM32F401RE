...
{   
	// Initialize mcp23s08 with all pins low (outputs)
	mcp23s08_init(CTRL_ADDR, 0x00);
  mcp23s08_init(DATA_ADDR, 0x00);
...
void ks0108_command(uint8_t command, uint8_t chipNo)
{
  ...
  mcp23s08_write(CTRL_ADDR, OLAT, ctrlCommands);          
	mcp23s08_write(DATA_ADDR, OLAT, command);
...
void ks0108_data(uint8_t data, uint8_t chipNo)
{
  ...
  mcp23s08_write(CTRL_ADDR, OLAT, ctrlCommands);          
	mcp23s08_write(DATA_ADDR, OLAT, data);
...
{
	mcp23s08_write(CTRL_ADDR, OLAT, ctrlData | EN_HIGH);
	mcp23s08_write(CTRL_ADDR, OLAT, ctrlData & EN_LOW);
}
//------------------------------------------------------------------------------
